...
void ft_julia(void *env, int pix_id)
{
	...
	if (info->psycho == 0)
	{
		...
		info->fr = fr;
		fr.c.real = fr_lint(-fr.wid_x / 2, fr.wid_x / 2, info->mouse.x / info->w);
		fr.c.real = fr_lint(-fr.wid_y / 2, fr.wid_y / 2, info->mouse.y / info->h);
		fr.z.real = fr_lint(fr.min_x, fr.max_x, (float)fr.x / info->w);
		fr.z.imag = fr_lint(fr.min_y, fr.max_y, (float)fr.y / info->h);
